...
export default function ConsumerMessages() {
  ...
  const [contactFilter, setContactFilter] = useState<'farmer' | 'delivery'>('farmer')
  ...
  const fetchContacts = async () => {
    if (!userId) return
    try {
      const response = await api.get(endpoints.chat.contacts, { params: { userId, role: 'consumer' } })
      ...
      console.error('Failed to load consumer chat contacts:', error)
      setContacts([])
    }
  }
  ...
  const fetchMessages = async (contactId: string) => {
    ...
    try {
      ...
      console.error('Failed to load consumer messages:', error)
      setMessages([])
    }
  }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      console.error('Failed to send consumer message:', error)
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <Button size="sm" variant={contactFilter === 'farmer' ? 'primary' : 'outline'} onClick={() => setContactFilter('farmer')}>{t('nav.farmers')}</Button>
