...
const menuItems = [
  ...
  { labelKey: 'nav.certificate', descriptionKey: 'farmer.certDetails', icon: FileCheck2, path: '/farmer/certificate' },
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      <Card className="bg-surface-elevated">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0"><h1 className="flex items-center gap-2 truncate font-[family-name:var(--font-display)] text-2xl font-bold">{userName}{certificateStatus === 'verified' ? <span className="inline-flex" aria-label={t('common.verified')} title={t('common.verified')}><CheckCircle2 className="h-5 w-5 shrink-0 text-primary" /></span> : <span className="inline-flex" aria-label={certificateLabel} title={certificateLabel}><XCircle className="h-5 w-5 shrink-0 text-accent" /></span>}</h1><p className="mt-1 truncate text-sm text-muted">{userEmail || t('common.email')}</p><Badge variant={certificateStatus === 'verified' ? 'success' : 'warning'} className="mt-3">{certificateLabel}</Badge></div>
        </div>
      </Card>
