...
export default function DeliveryMessages() {
  ...
  const [contactFilter, setContactFilter] = useState<'farmer' | 'consumer'>('farmer')
  ...
  const fetchContacts = async () => {
    if (!userId) return
    try {
      const response = await api.get(endpoints.chat.contacts, { params: { userId, role: 'delivery' } })
      const next = Array.isArray(response.data.contacts) ? response.data.contacts.map((c: any) => ({ ...c, id: String(c.id) })) : []
      setContacts(next)
      const requestedContact = searchParams.get('contactId')
      if (requestedContact && next.some((contact: any) => contact.id === requestedContact)) setActiveId(requestedContact)
      else if (!activeId && next[0]) setActiveId(next[0].id)
    } catch (error) {
      console.error('Failed to load delivery chat contacts:', error)
      setContacts([])
    }
  }
  ...
  const fetchMessages = async (contactId: string) => {
    ...
    try {
      ...
      console.error('Failed to load delivery messages:', error)
      setMessages([])
    }
  }
  ...
  }, [userId, activeContact, searchParams])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2"><Button size="sm" variant={contactFilter === 'farmer' ? 'primary' : 'outline'} onClick={() => setContactFilter('farmer')}>{t('nav.farmers')}</Button><Button size="sm" variant={contactFilter === 'consumer' ? 'primary' : 'outline'} onClick={() => setContactFilter('consumer')}>{t('nav.consumers')}</Button></div>
